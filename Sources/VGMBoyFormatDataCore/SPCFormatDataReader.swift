...
/// Reads SPC ID666 and xID6 metadata without starting an emulator.
public enum SPCFormatDataReader {
    ...
    public static func read(data: Data, displayName: String = "SPC") throws -> FormatMetadata {
        guard data.count >= minimumFileSize,
              data.prefix(headerMagic.count).elementsEqual(headerMagic) else {
            throw FormatDataError.malformed("Not an SPC file with a valid header: \(displayName)")
        }
        ...
        let playLengthMs = legacy.playLengthMs > 0
            ? legacy.playLengthMs
            : hasMetadataTags ? effectiveExtendedPlayLength(
