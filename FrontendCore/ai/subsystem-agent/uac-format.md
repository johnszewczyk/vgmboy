...
- The AudioMan CLI adds a `package` transformation for each linked source,
...
- New payloads use `payload.format` `tar+zstd-seekable` and a compression
  ...
  AudioMan packer defaults to level 3 and 4 MiB frames for the first SPC player
...
- `member.blake3` identifies exact stored member bytes. `streamBlake3` is a
  ...
  rendered-audio hash. In the current AudioMan profile, `.vgz` is hashed after
  gzip decompression; other playable formats hash their complete native member
  bytes, including headers/tags. No command-stream normalization or emulation
...
- The structural envelope is fixed and versioned, with a small stable set of
  ...
  survive read/write unchanged. MetaMan owns native-tag reading and eventual
  writing; UACMAN consumes its versioned machine-readable output once that
  contract is defined, rather than depending on an individual format reader
  such as the current SPC implementation.
...
V1 uses the existing TAR and Zstandard formats internally. The AudioMan CLI
...
compression does not alter TAR offsets or payload hashes. `UACContainerCore`
...
cache. Other formats whose decoders require filesystem paths still use the
existing selected-entry materialization route. AudioMan has a format-neutral CLI pack/inspect/unpack prototype that
uses a manually supplied JSON recipe; it does not parse native SPC/VGM tags.
Decoders that require a filesystem path still need a separate adapter or
explicit format-specific fallback. CLI unpack restores original members and
writes `manifest.json` by default; it does not synthesize an M3U when no
original playlist member was included.

## Ownership and Safety

- `UACContainerCore` in FrontendCore owns the bounded manifest reader,
...
- AudioMan owns the collection packer and source-to-UAC provenance mapping.
...
- ScanSong remains the only catalog writer. Its UAC indexing must consume the
  same manifest contract without granting player apps catalog write access.
