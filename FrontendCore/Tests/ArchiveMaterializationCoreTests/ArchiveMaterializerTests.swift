...
@Test func playbackMaterializerStagesAnMDXArchiveDependencyBesideTheSelectedMember() throws {
    let root = FileManager.default.temporaryDirectory
        .appendingPathComponent("ArchivePlaybackMDXTests-\(UUID().uuidString)", isDirectory: true)
    let source = root.appendingPathComponent("source", isDirectory: true)
    let archive = root.appendingPathComponent("library.tar")
    let cache = root.appendingPathComponent("cache", isDirectory: true)
    let keys = ArchiveCachePreferenceKeys(
        modeKey: "ArchivePlaybackMDXTests.\(UUID().uuidString).mode",
        limitKey: "ArchivePlaybackMDXTests.\(UUID().uuidString).limit"
    )
    try FileManager.default.createDirectory(at: source, withIntermediateDirectories: true)
    let mdx = Data("title\r\n\u{001A}BANK.PDX\0payload".utf8)
    try mdx.write(to: source.appendingPathComponent("track.MDX"))
    try Data("pdx-payload".utf8).write(to: source.appendingPathComponent("BANK.PDX"))
    defer { try? FileManager.default.removeItem(at: root) }
    ...
    let dependency = selected.deletingLastPathComponent().appendingPathComponent("BANK.PDX")
    #expect(try Data(contentsOf: dependency) == Data("pdx-payload".utf8))
    materializer.release()
}

@Test func playbackMaterializerStagesACompressedStandaloneMDXDependency() throws {
    let root = FileManager.default.temporaryDirectory
        .appendingPathComponent("ArchivePlaybackStandaloneMDXTests-\(UUID().uuidString)", isDirectory: true)
    let source = root.appendingPathComponent("source", isDirectory: true)
    let mdxSource = source.appendingPathComponent("track.MDX")
    let pdxSource = source.appendingPathComponent("BANK.PDX")
    let mdxArchive = source.appendingPathComponent("track.MDX.zst")
    let pdxArchive = source.appendingPathComponent("BANK.PDX.zst")
    let cache = root.appendingPathComponent("cache", isDirectory: true)
    let keys = ArchiveCachePreferenceKeys(
        modeKey: "ArchivePlaybackStandaloneMDXTests.\(UUID().uuidString).mode",
        limitKey: "ArchivePlaybackStandaloneMDXTests.\(UUID().uuidString).limit"
    )
    try FileManager.default.createDirectory(at: source, withIntermediateDirectories: true)
    try Data("title\r\n\u{001A}BANK.PDX\0payload".utf8).write(to: mdxSource)
    try Data("pdx-payload".utf8).write(to: pdxSource)
    defer { try? FileManager.default.removeItem(at: root) }
    ...
    for (input, output) in [(mdxSource, mdxArchive), (pdxSource, pdxArchive)] {
        let compressor = Process()
        compressor.executableURL = URL(fileURLWithPath: zstdPath)
        compressor.arguments = ["-q", "-f", input.path, "-o", output.path]
        try compressor.run()
        compressor.waitUntilExit()
        #expect(compressor.terminationStatus == 0)
    }
    try FileManager.default.removeItem(at: mdxSource)
    try FileManager.default.removeItem(at: pdxSource)
