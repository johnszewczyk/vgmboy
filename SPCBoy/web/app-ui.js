...
function databaseRowsToPlaylistTracks(rows, games) {
  ...
  return rows.map((row, index) => ({
    ...
    // Catalog rows already have their authoritative database projection. A
    // missing metadataTrackId means "unknown", not "scan this disk path".
    // Playback may inspect one selected track if it needs a missing duration;
    // playlist loading must never fan that work out over the whole catalog.
    catalogRow: true
  }));
}
...
function renderPlaylist({ sort = true } = {}) {
  ...
  // Measuring every cell for auto-sizing defeats the point of a catalog
  // lookup. Large database playlists keep the user's established widths;
  // explicit column auto-size actions still measure on demand.
  const playlistSignature = virtualized ? null : playlistAutoSizeSignature();
  const shouldAutoSize = !virtualized && state.columnAutoSize && playlistSignature !== autoSizedPlaylistSignature;
  ...
  const firstVisibleRow = virtualized
    ? Math.max(0, Math.floor(scrollTop / playlistVirtualRowHeight) - PLAYLIST_VIRTUAL_OVERSCAN)
    : 0;
  const lastVisibleRow = virtualized
    ? Math.min(state.playlist.length, Math.ceil((scrollTop + viewportHeight) / playlistVirtualRowHeight) + PLAYLIST_VIRTUAL_OVERSCAN)
    : state.playlist.length;
  const fragment = document.createDocumentFragment();
  if (virtualized && firstVisibleRow > 0) {
    fragment.appendChild(makePlaylistVirtualSpacer(firstVisibleRow * playlistVirtualRowHeight));
  }
  ...
  if (virtualized && lastVisibleRow < state.playlist.length) {
    fragment.appendChild(makePlaylistVirtualSpacer((state.playlist.length - lastVisibleRow) * playlistVirtualRowHeight));
  }
...
function moveSelection(delta) {
  ...
  if (playlistUsesVirtualRows() && !playlistRowsByTrackId.has(state.selectedTrackId)) {
    refs.playlistBodyWrap.scrollTop = Math.max(0, nextIndex * playlistVirtualRowHeight - (refs.playlistBodyWrap.clientHeight / 2));
    renderPlaylist({ sort: false });
    selectPlaylistTrack(state.selectedTrackId, { focus: true });
  }
