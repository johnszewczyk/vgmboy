...
protected:
	...
	static const uint32_t CHECK_SILENCE_LEVEL = 3;
	...
	bool playInfinitely, overrideLength;
...
public:
	...
	bool GetPlayInfinitely() const { return this->playInfinitely; }
	void SetSampleRate(double newSampleRate) { this->sampleRate = newSampleRate; }
	void SetVolumeType(VolumeType v, PeakType p);
	void IgnoreVolume() { this->ignoreVolume = true; }
	void PlayInfinitely() { this->playInfinitely = true; }
	void SetLength(int ms, int fade);
	virtual bool Load();
