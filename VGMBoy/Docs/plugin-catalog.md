...
Upstream revisions and SHA-256 digests for the flattened source snapshots are
recorded in [`vendor/PROVENANCE.md`](../vendor/PROVENANCE.md). These are ordinary
files in the family repository, not active Git submodules. Compatibility
...
its required library chain is present and resolvable. MetaMan reads the common
`[TAG]` footer for PSF/PSF2, SSF, USF, and 2SF without invoking those cores;
those plugins remain VGMBoy's playback engines. ScanSong independently
