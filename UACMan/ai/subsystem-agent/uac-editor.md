...
- `CanonicalTable.setFoldOpen`, `fillUnfold`, and `playPendingFoldAnimations`
  ...
  fold animations. Keep pending opens until content is present so a render
  caused by an asynchronous manifest read still animates the new table.
- `flat-table` and `giga-table` are presentation-mode modifiers on the same
  renderer; they do not own separate row markup or column templates. Folding
  is generated by `rowWithUnfolds`, not by a separate table type. The former
