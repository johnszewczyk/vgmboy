(() => {
  ...
  function renderTagScopePage(title, scope, includeAttachments = false) {
    ...
    const canonicalGrid = (gridTitle, rows, popups, gridScope) => `<section class="tag-scope-table"><div class="section-title"><span>${gridTitle}</span><span class="section-help">Package-level metadata</span></div>${gridScope === "package" ? createForm(gridScope) : ""}<div class="data-table-scroll canonical-table-surface"><div class="field-grid canonical-table flat-table meta-field-grid" role="table" aria-label="${gridTitle}">${header}${rows || '<div class="field-grid-empty" role="row"><span role="cell">No tags</span></div>'}</div></div>${popups.join("")}</section>`;
