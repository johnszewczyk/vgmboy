...
@Test("AY, SAP, and the registered single-track formats are direct readers")
func supportedFormatsAreRegisteredAsDirectFormatDataParsers() {
    let formats = MetaManCore.supportedFormats
    #expect(formats.map(\.identifier) == ["ay", "sap", "s98", "vgm", "psf-family", "spc", "sid", "ape", "adx", "aus", "at3", "msf", "svag"])
    #expect(formats[0].fileExtensions == ["ay"])
    #expect(formats[0].methodology.contains("no playback decoder"))
    #expect(formats[1].fileExtensions == ["sap"])
    #expect(formats[1].methodology.contains("ordered SONGS/TIME subtunes"))
    #expect(formats[2].fileExtensions == ["s98"])
    #expect(formats[2].methodology.contains("no playback decoder"))
    #expect(formats[4].fileExtensions.contains("mini2sf"))
    #expect(formats[5].fileExtensions == ["spc"])
    #expect(formats[5].methodology.contains("does not start the playback emulator"))
    #expect(formats[6].fileExtensions == ["sid"])
    #expect(formats[6].methodology.contains("does not instantiate the SID playback core"))
    #expect(formats[7].fileExtensions == ["ape"])
    #expect(formats[7].methodology.contains("without an audio decoder"))
    #expect(formats[8].fileExtensions == ["adx"])
    #expect(formats[8].methodology.contains("Direct CRI/Monster ADX"))
    #expect(formats[9].fileExtensions == ["aus"])
    #expect(formats[9].methodology.contains("Direct Atomic Planet"))
    #expect(formats[10].fileExtensions == ["at3"])
    #expect(formats[10].methodology.contains("RIFF/WAVE ATRAC3"))
    #expect(formats[11].fileExtensions == ["msf"])
    #expect(formats[11].methodology.contains("Direct Sony MSF"))
    #expect(formats[12].fileExtensions == ["svag"])
    #expect(formats[12].methodology.contains("Konami/SNK SVAG"))
}
