...
int mdx_open( t_mdxmini *data, char *filename , char *pcmdir )
{
  ...
  if (!self_construct(data->songdata)) {
    ...
    /* A declared PDX is required input. The historical player silently
       disabled PCM when the bank was absent, which produced deceptively
       playable but incomplete output. The host bridge needs a hard failure
       so scanner and playback agree about dependency validity. */
    if ( mdx->pdx_name[0] != '\0' && pdx == NULL ) {
        mdx_close(data);
        return -1;
    }
...
/* MDX collections commonly contain PDX names whose case differs from the
 * spelling embedded in the module. macOS's default filesystem is usually
 * case-insensitive, but archive materialization is not required to preserve
 * that behavior. Resolve only a basename in the intended directory and never
 * walk outside it. */
static PDX_DATA*
_open_pdx_case_insensitive(char* name)
{
  char directory[PATH_BUF_SIZE];
  char *slash = strrchr(name, '/');
  const char *basename = slash ? slash + 1 : name;
  size_t directory_length = slash ? (size_t)(slash - name + 1) : 0;
  if (directory_length >= sizeof(directory)) return NULL;
  if (directory_length > 0) memcpy(directory, name, directory_length);
  directory[directory_length] = '\0';

  DIR *dir = opendir(directory_length > 0 ? directory : ".");
  if (!dir) return NULL;
  struct dirent *entry;
  PDX_DATA *pdx = NULL;
  while ((entry = readdir(dir)) != NULL) {
    if (strcasecmp(entry->d_name, basename) != 0) continue;
    char candidate[PATH_BUF_SIZE];
    int written = snprintf(candidate, sizeof(candidate), "%s%s", directory, entry->d_name);
    if (written < 0 || (size_t)written >= sizeof(candidate)) break;
    pdx = _open_pdx(candidate);
    if (pdx) break;
  }
  closedir(dir);
  return pdx;
...
_get_pdx(MDX_DATA* mdx, char* mdxpath)
{
  ...
  {
    pdx = _open_pdx_case_insensitive(buf);
    if (pdx != NULL)
      goto get_pdx_file;
