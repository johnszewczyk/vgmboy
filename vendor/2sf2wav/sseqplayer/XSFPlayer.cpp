...
XSFPlayer::XSFPlayer() : xSF(), sampleRate(0), detectedSilenceSample(0), detectedSilenceSec(0), skipSilenceOnStartSec(5), lengthSample(0), fadeSample(0), currentSample(0),
	prevSampleL(CHECK_SILENCE_BIAS), prevSampleR(CHECK_SILENCE_BIAS), lengthInMS(-1), fadeInMS(-1), volume(1.0), volumeType(VOLUMETYPE_REPLAYGAIN_ALBUM),
	peakType(PEAKTYPE_REPLAYGAIN_TRACK), ignoreVolume(false), uses32BitSamplesClampedTo16Bit(false), playInfinitely(false)
{
}
...
{
	bool endFlag = false;
	unsigned detectSilence = 0; // xSFConfig->GetDetectSilenceSec();
	...
	/* Volume */
	if (!this->ignoreVolume && (!fEqual(this->volume, 1.0) || !fEqual(1.0 /*xSFConfig->GetVolume()*/, 1.0)))
	{
		double scale = this->volume * 1.0 /*xSFConfig->GetVolume()*/;
	...
	/* Fading */
	if (!this->playInfinitely && this->fadeSample && this->currentSample + bufsize >= this->lengthSample)
	{
		auto bufShort = reinterpret_cast<int16_t *>(&buf[0]);
		for (unsigned ofs = 0; ofs < bufsize; ++ofs)
		{
...
void XSFPlayer::SetLength(int ms, int fade)
{
	this->overrideLength = true;
	this->lengthInMS = ms;
	this->fadeInMS = fade;
}
...
{
	if (this->xSF) {
		this->volume = this->xSF->GetVolume(volumeType, peakType);
		if (!this->overrideLength) {
			this->lengthInMS = this->xSF->GetLengthMS(115 * 1000 /* xSFConfig->GetDefaultLength() */);
			this->fadeInMS = this->xSF->GetFadeMS(5000 /*xSFConfig->GetDefaultFade()*/);
		}
	}
...
void XSFPlayer::SetVolumeType(VolumeType v, PeakType p)
{
	this->volumeType = v;
	this->peakType = p;
	if (this->xSF) {
...
{
	this->skipSilenceOnStartSec = 0; // xSFConfig->GetSkipSilenceOnStartSec();
