...
column resizing is bidirectional and uses the shared configurable 200ms default
elapsed-time animation contract with ease-in-out timing. Auto-fit updates on
display-synchronized frames and persists at the target; it does not use a
fixed step count or arbitrary duration cap. The resize gesture uses the Electron structure exactly:
